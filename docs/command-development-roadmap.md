...
| `veeamgo restore instant-vm start|unmount` | Manage instant recovery | Provide target/cleanup flags |
| `veeamgo restore flr start|validate|stop` | Control file-level recovery sessions | Mask secrets in output |
...
| `veeamgo license install|remove` | Manage license files | Validate checksum, prompt confirmation |
