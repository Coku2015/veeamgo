...
- **Authentication & Sessions**
  - `login` / `logout` / `get session` commands operational on macOS, Linux, and Windows.
...
- **Server Insights**
  - `veeamgo get server info` / `veeamgo get server time` emit redesigned single-record views with timezone detection fixes.
- **Infrastructure Inventory**
  ...
  - Inventory browsers: `veeamgo get inventory virtualinfra|unstructured|protectiongroup` tap the stable POST endpoints with pagination and optional JSON filters.
...
- **Stage 3B UX Polish**
  ...
  - `restorepoint`/`replica` commands now require explicit job context, output job-aware tables, and remove redundant ID-heavy columns; legacy restore/mount commands remain retired while malware detection visibility returns via `veeamgo get malwaredetectionevent` and `veeamgo get yararule`.
