...
- **Help text separation**: Follow the conventions in `docs/managing-text-and-description.md`. All Cobra `Short`, `Long`, and `Example` fields must live in `pkg/helptext/` (one file per command grouping). Command files should import these constants so that logic and text remain isolated, i18n-ready, and documentation-friendly.
...
## 12. API Version Compatibility Strategy
- Always include `x-api-version` on requests. Default to the latest version bundled with the repo (currently `1.3-rev0`) and allow overrides via CLI/config.
- Cache the negotiated revision inside the profile after the first successful `serverInfo` call so subsequent commands reuse it without incurring an extra round-trip. Warn when the server is newer than the client supports.
- Maintain a Go feature matrix keyed by API version to gate commands (e.g., SOBR operations, mount server APIs). If a capability is unavailable the CLI must emit a clear “not supported by this server version” error instead of calling the API.
- Treat missing JSON fields as optional (`omitempty`) so older responses do not break parsing. New write operations should be hidden behind the feature gate to avoid `4xx` responses on legacy servers.
- Extend smoke tests to cover at least the latest release plus one prior version. Allow `scripts/veeamgo_e2e.sh` to accept `API_VERSION` and skip steps when the feature matrix says “unsupported”.
- Document minimum API requirements for every new command and highlight version bumps in release notes.
