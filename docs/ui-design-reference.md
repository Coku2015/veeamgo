...
The `--type` flag accepts `vmware`, `hyperv`, or `general`, which map to the API types `ViProxy`, `HvProxy`, and `GeneralPurposeProxy` shown in detailed output.
...
Registers a Linux managed server. All parameters must be supplied explicitly.
...
Flags:
      --credentials-id string           Existing credentials ID to reuse
      --credentials-storage string      Credentials storage type (Permanent, SingleUse, Certificate)
      --description string              Description for the managed server
      --handshake-code string           Handshake code for certificate-based pairing
  -h, --help                            help for linux
      ...
      --single-use-auth-type string     Authentication type for single-use credentials
      ...
      --single-use-private-key string   Private key (PEM) for single-use credentials
      --single-use-root-password string Root password for privilege escalation
      --single-use-ssh-port int         SSH port for single-use credentials
      --single-use-use-su               Use su instead of sudo in single-use mode
      --single-use-username string      Username for single-use credentials
...
user@host % veeamgo add managedserver linux \
    --name repo01.lab.local \
    --description "Repository gateway" \
    --credentials-storage Permanent \
    --ssh-fingerprint "ssh-rsa 3072 AAAA..." \
    --wait --yes
Linux managed server "repo01.lab.local" provisioning finished with result Success (session 3bd08254-3b6f-4220-9410-cb7d19fdb7cf).
Managed server ID: 0c8d2af4-7922-4de3-9efd-9e0d9982f8d1
```

**Certificate re-pair (`--wait`)**
```
user@host % veeamgo add managedserver linux \
    --name repo02.lab.local \
    --description "Veeam appliance" \
    --credentials-storage Certificate \
    --handshake-code 123456 \
    --ssh-fingerprint "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAA..." \
    --wait --yes
Linux managed server "repo02.lab.local" provisioning finished with result Success (session 8c5fbb6e-23e2-4f9e-85a1-c3fd41464a12).
Managed server ID: 6e598c57-2b48-4de3-9efd-9e0d9982f8d1
```
...
**Help**
```
user@host % veeamgo rescan managedserver --help
Rescan managed servers

Usage:
  veeamgo rescan managedserver [flags]

Flags:
      --all           Rescan all managed servers
  -h, --help          help for managedserver
      --id string     Managed server ID to rescan
      --name string   Managed server name to rescan
      --wait          Wait for the rescan session to finish
...
**Success (single)**
```
user@host % veeamgo rescan managedserver --name repo01
Started rescan for Managed Server "repo01" (session 3cf4a868-56cb-4f3a-9134-adc7417f2195).
Use veeamgo get session logs --id 3cf4a868-56cb-4f3a-9134-adc7417f2195 to track progress.
```

**Success (wait, all)**
```
user@host % veeamgo rescan managedserver --all --wait
Managed servers rescan finished with result Success (session 0b9e3ad2-89d4-4be4-9c74-2f0cc6d5e8c3).
```

**Failure**
```
user@host % veeamgo rescan managedserver --name repo01 --wait
Managed Server "repo01" rescan finished with result Failed (session 12ef3b2d-4829-4e4d-8d09-6f517b83c6d0).
Host is unreachable. Check network connectivity and credentials, then retry.
```

### `veeamgo start publishdisk`
> The CLI waits briefly (up to about two minutes) for the publishing session to finish. When the mount becomes available quickly, the published disk details are printed automatically. The command currently emits iSCSI targets only; additional publishing modes will surface once the API defects are cleared.


**Help**
```
user@host % veeamgo start publishdisk --help
Start publishing backup disks

Usage:
  veeamgo start publishdisk [flags]

Flags:
      --allowed-ip strings  Allowed IP address for iSCSI target mode (repeatable)
      --disk strings        Restore point disk name to publish (repeatable)
  -h, --help                help for publishdisk
      --restore-point-id string Restore point ID to publish
      --wait                Wait for the disk publishing session to finish
...
**Success (`--wait`)**
```
user@host % veeamgo start publishdisk \
    --restore-point-id 58d8ce8c-2e04-47c7-8f11-a17875b1eed3 \
    --allowed-ip 10.50.20.15 \
    --allowed-ip 10.50.20.16 \
    --wait
Disk publishing for restore point "linbase02 AgentBackupJob" finished with result Success (mode ISCSITarget, session 801fe1f4-0f2f-4b42-93fd-5f6bc9a1c170).
Id:                     ac6b9f4e-dbc6-4cfe-8be3-3c4b5c9ee4aa
Restore Point:          linbase02 AgentBackupJob
Backup:                 AgentBackupJob
Mode:                   ISCSITarget
State:                  Published
Target:                 winmount01.lab.local
IPs:                    10.50.20.27
Port:                   2049
Restore Point Id:       58d8ce8c-2e04-47c7-8f11-a17875b1eed3
Session Id:             801fe1f4-0f2f-4b42-93fd-5f6bc9a1c170
Error:
```
...
user@host % veeamgo start publishdisk \
    ...
    --allowed-ip 10.50.20.15 \
    --wait
Disk publishing for restore point "linbase02 AgentBackupJob" finished with result Failed (mode ISCSITarget, session d49b2ce6-3912-4e33-8d4f-e1abc8a22211).
No available mount server to handle iSCSI publishing. Configure a compatible mount server and retry.
```
...
**Help**
```
user@host % veeamgo stop publishdisk --help
Stop published disks

Usage:
  veeamgo stop publishdisk [flags]

Flags:
      --id strings  Published disk mount ID to stop (repeatable)
  -h, --help        help for publishdisk
      --wait        Wait for the unpublish session to finish
...
**Success (`--wait`)**
```
user@host % veeamgo stop publishdisk --id ac6b9f4e-dbc6-4cfe-8be3-3c4b5c9ee4aa --wait
Stopped published disk ac6b9f4e-dbc6-4cfe-8be3-3c4b5c9ee4aa with result Success (session 5f89a9e2-6a2f-4cb0-87c9-387ed2a6496f).
```

**Success (async)**
```
user@host % veeamgo stop publishdisk --id df9a2b13-4f45-4bb5-9b42-0d7a7c9f8e30
Stopping published disk df9a2b13-4f45-4bb5-9b42-0d7a7c9f8e30 (session 2cf88054-4a6a-4dea-b0cb-7266cfe33052).
Use veeamgo get session logs --id 2cf88054-4a6a-4dea-b0cb-7266cfe33052 to track progress.
```

**Failure**
```
user@host % veeamgo stop publishdisk --id bad-id --wait
Failed to stop published disk bad-id: mount id cannot be empty
```

### `veeamgo get publisheddisk`

**Help**
```
user@host % veeamgo get publisheddisk --help
List published disks

Usage:
  veeamgo get publisheddisk [flags]

Flags:
      --all        List all published disks
  -h, --help       help for publisheddisk
      --id string  Published disk mount ID to describe
      --limit int  Maximum number of mounts to return when using --all
...
**Success (`--all`)**
```
user@host % veeamgo get publisheddisk --all --limit 2
Id                                   Restore Point               Mode              State        Target               IPs                Port  Error
ac6b9f4e-dbc6-4cfe-8be3-3c4b5c9ee4aa  linbase02 AgentBackupJob    FUSELinuxMount    Published    linbase03.lab.local  10.50.20.27        2049
df9a2b13-4f45-4bb5-9b42-0d7a7c9f8e30  linbase02 AgentBackupJob    ISCSITarget       Published    winmount01.lab.local 192.168.1.150, 192.168.1.151  3260
```

**Success (`--id`)**
```
user@host % veeamgo get publisheddisk --id ac6b9f4e-dbc6-4cfe-8be3-3c4b5c9ee4aa
Id:                     ac6b9f4e-dbc6-4cfe-8be3-3c4b5c9ee4aa
Restore Point:          linbase02 AgentBackupJob
Backup:                 AgentBackupJob
Mode:                   FUSELinuxMount
State:                  Published
Initiator:              linbase03.lab.local
IPs:                    10.50.20.27
Port:                   2049
Restore Point Id:       58d8ce8c-2e04-47c7-8f11-a17875b1eed3
Session Id:
...
**Error**
```
user@host % veeamgo get publisheddisk
Error: provide --id or use --all
```
...
**Help**
```
user@host % veeamgo delete managedserver --help
Delete a managed server

Usage:
  veeamgo delete managedserver [flags]

Flags:
      --id string     Managed server ID to delete
  -h, --help          help for managedserver
      --name string   Managed server name to delete
      --wait          Wait for the deletion session to finish
...
**Success (start)**
```
user@host % veeamgo delete managedserver --name repo01
Managed Server "repo01" deletion started (session 9b3b4af7-9a43-40f8-aa2c-73c9c6a41f25).
Use veeamgo get session logs --id 9b3b4af7-9a43-40f8-aa2c-73c9c6a41f25 to track progress.
```

**Success (wait)**
```
user@host % veeamgo delete managedserver --name repo01 --wait --yes
Managed Server "repo01" deletion finished with result Success (session 9b3b4af7-9a43-40f8-aa2c-73c9c6a41f25).
```

**Failure**
```
user@host % veeamgo delete managedserver --name sobr-gateway --wait --yes
Managed Server "sobr-gateway" deletion finished with result Failed (session 8a6e32d8-69db-4086-b8b9-1e2b3a4cdc73).
The server is still referenced by backup repository "Hardened SOBR Repository". Detach the repository and retry.
```
...
**Help**
```
user@host % veeamgo add repository --help
Add a backup repository

Usage:
  veeamgo add repository [flags]

Flags:
      --credentials-id string        Credentials ID for SMB share access
      --description string           Repository description
      --disable                      Create the repository in a disabled state
      --fast-clone                   Enable fast cloning on XFS volumes (LinuxLocal, LinuxHardened)
  -h, --help                         help for repository
      --host string                  Managed server host name or ID (WinLocal, LinuxLocal, LinuxHardened)
      --immutable-days int           Number of days to keep recent backups immutable (LinuxHardened)
      --import-backup                Search the repository for existing backups after creation
      --mount-server string          Mount server name or ID
      --mount-server-cache string    Path for vPower write cache on the mount server
      --mount-server-nfs-port int    vPower NFS port (default 2049)
      --mount-server-vpower          Enable vPower NFS on the mount server (default true)
      --mount-server-mount-port int  Mount port for vPower NFS (default 1058)
      --name string                  Repository name
      --path string                  Local path for storing backups (WinLocal, LinuxLocal, LinuxHardened)
      --share-path string            UNC or NFS share path (SMB, NFS)
      --type string                  Repository type (WinLocal, LinuxLocal, LinuxHardened, SMB, NFS)
      --wait                         Wait for the repository provisioning session to finish
      --yes                          Confirm without prompting (required to execute)

      --gateway-auto                 Automatically select a gateway server for SMB/NFS repositories (default true)
      --gateway-server strings       Gateway server name or ID (repeatable)
      --max-tasks int                Maximum concurrent tasks (0 disables the limit)
      --read-write-limit int         Read/write speed limit in MB/s (0 disables the limit)
      --import-index                 Import guest OS file system index when importing backups
...
**Success**
```
user@host % veeamgo add repository --type WinLocal --host repo-win01 --path "D:\Backups" --mount-server repo-win01 --name "Windows Repository" --yes
Started provisioning Windows Local Repository "Windows Repository" on host "repo-win01" (session 7a59ddfa-4b3f-46ac-9a0a-d2db1d69fd4b).
Use veeamgo get session logs --id 7a59ddfa-4b3f-46ac-9a0a-d2db1d69fd4b to track progress.
```

**Error**
```
user@host % veeamgo add repository --type WinLocal --host repo-win01
Error: flag --path is required (provide the repository path)
Usage:
  veeamgo add repository [flags]
```
...
**Help**
```
user@host % veeamgo edit repository --help
Edit a backup repository

Usage:
  veeamgo edit repository [flags]

Flags:
      --credentials-id string        Credentials ID for SMB share access
      --description string           New repository description
      --disable                      Disable the repository (set to true or false)
      --fast-clone                   Enable fast cloning on XFS volumes (LinuxLocal, LinuxHardened)
  -h, --help                         help for repository
      --host string                  Managed server host name or ID (WinLocal, LinuxLocal, LinuxHardened)
      --id string                    Repository ID to edit
      --immutable-days int           Number of days to keep recent backups immutable (LinuxHardened)
      --import-backup                Search the repository for existing backups
      --import-index                 Import guest OS file system index when importing backups
      --max-tasks int                Maximum concurrent tasks (0 disables the limit)
      --mount-server string          Mount server name or ID
      --mount-server-cache string    Path for vPower write cache on the mount server
      ...
      --mount-server-nfs-port int    vPower NFS port (default 2049)
      --mount-server-vpower          Enable vPower NFS on the mount server (default true)
      --name string                  Repository name to edit
      --new-name string              New repository name
      --path string                  Local path for storing backups (WinLocal, LinuxLocal, LinuxHardened)
      --share-path string            UNC or NFS share path (SMB, NFS)
      --type string                  Expected repository type (optional)
      --wait                         Wait for the repository reconfiguration session to finish
      --yes                          Confirm without prompting (required to execute)

      --gateway-auto                 Automatically select a gateway server for SMB/NFS repositories
      --gateway-server strings       Gateway server name or ID (repeatable)
      --read-write-limit int         Read/write speed limit in MB/s (0 disables the limit)
...
**Success**
```
user@host % veeamgo edit repository --name "Windows Repository" --path "E:\Backups" --max-tasks 6 --wait --yes
Windows Local Repository "Windows Repository" on host "repo-win01" update finished with result Success (session 0f32af7d-35fd-4a1f-8b3b-2f51de4c9cf1).
Repository ID: 88788f9e-d8f5-4eb4-bc4f-9b3f5403bcec
```

**Error**
```
user@host % veeamgo edit repository --name "Windows Repository"
Windows Local Repository "Windows Repository" on host "repo-win01" (path D:\Backups) left unchanged.
```

### `veeamgo rescan repository`

**Help**
```
user@host % veeamgo rescan repository --help
Rescan one or more repositories

Usage:
  veeamgo rescan repository [flags]

Flags:
      --all               Rescan every repository
  -h, --help              help for repository
      --id strings        Repository ID to rescan (can be repeated)
      --name strings      Repository name to rescan (can be repeated)
      --wait              Wait for the rescan session to finish
...
**Success**
```
user@host % veeamgo rescan repository --name "Default Backup Repository"
id:                52dd4db8-4cb6-4710-8f09-2c4aa1e6d5ee
name:              Repository rescan
state:             Running
result:
progressPercent:   0
creationTime:      2025-10-17 11:30:00
```

**Error**
```
user@host % veeamgo rescan repository
Error: provide at least one --id, --name, or use --all
Usage:
  veeamgo rescan repository [flags]
```

### `veeamgo delete repository`

**Help**
```
user@host % veeamgo delete repository --help
Delete a backup repository

Usage:
  veeamgo delete repository [flags]

Flags:
      --delete-backups   Also remove backup files stored in the repository
  -h, --help              help for repository
      --id string         Repository ID to delete (optional)
      --name string       Repository name to delete (optional)
      --yes               Confirm without prompting (required to execute)
...
**Success**
```
user@host % veeamgo delete repository --name "Default Backup Repository" --delete-backups --yes
Deleted Linux Hardened Repository "Default Backup Repository" (88788f9e-d8f5-4eb4-bc4f-9b3f5403bcec) and removed backup files.
```

**Error**
```
user@host % veeamgo delete repository --delete-backups
Delete Linux Hardened Repository "Default Backup Repository" (including backup files) [y/N]: n
Cancelled deleting Linux Hardened Repository "Default Backup Repository".
```

### `veeamgo get objectrepository`

**Help**
```
user@host % veeamgo get objectrepository --help
List object storage repositories

Usage:
  veeamgo get objectrepository [flags]

Flags:
      --limit int        Maximum number of repositories to return (default: all)
      --name string      Filter by repository name (supports * wildcards)
      --type strings     Filter by repository type (e.g. AmazonS3, S3Compatible, AzureBlob)
  -h, --help             help for objectrepository
...
**Success**
```
user@host % veeamgo get objectrepository --limit 2
Name                     Type        Description                                   Online  Id
AWS Repository           Amazon S3   S3 bucket for archival backups                Yes     1b6e9ba6-0739-4ae3-9c2a-1c5d57eafc4f
S3 Compatible Repository S3 Compat   S3 compatible off-site repository             No      0ad5e9c2-0173-44cc-91b2-51bc0f8742df
```

**Error**
```
user@host % veeamgo get objectrepository --type WinLocal
Error: object repository type "WinLocal" is not supported
Usage:
  veeamgo get objectrepository [flags]
```

### `veeamgo describe objectrepository`

**Help**
```
user@host % veeamgo describe objectrepository --help
Describe an object storage repository

Usage:
  veeamgo describe objectrepository [flags]

Flags:
  -h, --help          help for objectrepository
      --name string   Object storage repository name to describe
...
**Success**
```
user@host % veeamgo describe objectrepository --name "AWS Repository"
Name:        AWS Repository
Type:        Amazon S3
Description: S3 bucket for archival backups
Online:      Yes
Id:          1b6e9ba6-0739-4ae3-9c2a-1c5d57eafc4f
Config:
  account:
    credentialsId: 4c4dd51a-6d4e-42ed-965d-24e582cfb9a2
    regionType: Global
  bucket:
...
**Error**
```
user@host % veeamgo describe objectrepository --name "Default Backup Repository"
Error: repository "Default Backup Repository" is not an object storage repository; use veeamgo describe repository
```
...
**Help**
```
user@host % veeamgo add objectrepository --help
Add an object storage repository

Usage:
  veeamgo add objectrepository [flags]
  veeamgo add objectrepository [command]

Available Commands:
  amazon-s3               Add an Amazon S3 repository
  azure-blob              Add an Azure Blob repository
  azure-archive           Add an Azure Archive repository
  s3-compatible           Add an S3 Compatible repository
  wasabi-cloud            Add a Wasabi Cloud repository
  types                   List supported object repository types
  veeam-data-cloud-vault  Add a Veeam Data Cloud Vault repository

Flags:
      --description string   Override repository description
      --disable              Create the repository in a disabled state
  -h, --help                 help for objectrepository
      --name string          Override repository name
      --set stringArray      Override spec value (repeatable, dot notation)
      --spec string          Path to JSON spec describing the repository (use '-' for stdin)
      --type string          Object repository type (AzureBlob, AmazonS3, S3Compatible, ...)
      --wait                 Wait for the provisioning session to finish
      --yes                  Confirm without prompting (required to execute)
...
**Provider success**
```
user@host % veeamgo add objectrepository amazon-s3 \
    --name "Archive Bucket" \
    --credentials-id 2aaed9b2-5d60-4be7-9f8b-cb5a46a2f84a \
    --bucket-name company-archive \
    --folder veeam \
    --region-id us-east-1 \
    --mount-server-id 6a7c4593-f2da-4ff7-9cb0-8fdc21090278 \
    --mount-server-cache "D:\VeeamCache" \
    --yes
Started provisioning Amazon S3 repository "Archive Bucket" target bucket company-archive/veeam (session 1fbf3b13-8f1a-4273-9fb7-6c5a08f8d94d).
Use veeamgo get session logs --id 1fbf3b13-8f1a-4273-9fb7-6c5a08f8d94d to track progress.
```

**Wasabi success**
```
user@host % veeamgo add objectrepository wasabi-cloud \
    --name "Wasabi Repo" \
    --credentials-id 1904884c-4a5d-4a72-9158-6a0e8f3df210 \
    --region-id us-east-1 \
    --bucket-name company-archive \
    --folder veeam \
    --mount-server-id 6a7c4593-f2da-4ff7-9cb0-8fdc21090278 \
    --mount-server-cache "D:\VeeamCache" \
    --yes
Started provisioning Wasabi Cloud repository "Wasabi Repo" target bucket company-archive/veeam (session 6d671369-0d58-4ad4-9e63-b0232a7bbce2).
Use veeamgo get session logs --id 6d671369-0d58-4ad4-9e63-b0232a7bbce2 to track progress.
```

**Azure Archive success**
```
user@host % veeamgo add objectrepository azure-archive \
    --name "Archive Tier" \
    --credentials-id 7aa17809-0e52-4f23-992f-3a7420530c21 \
    --container-name cold-storage \
    --folder vault \
    --proxy-subscription-id 514c0221-5a0e-4b43-b49a-946f2a6d4872 \
    --proxy-resource-group veeam-proxies \
    --proxy-virtual-network veeam-vnet \
    --proxy-subnet proxy-subnet \
    --mount-server-id 9ce5bfa1-7f73-4102-9641-66980b9e8d7f \
    --mount-server-cache "D:\Cache" \
    --yes
Started provisioning Azure Archive repository "Archive Tier" target container cold-storage/vault (session 932e72c3-9392-42f8-9f4f-96f57d6e3111).
Use veeamgo get session logs --id 932e72c3-9392-42f8-9f4f-96f57d6e3111 to track progress.
```

**Raw spec success**
```
user@host % veeamgo add objectrepository --type AmazonS3 --spec ./s3.json --set bucket.bucketName=prod-archives --yes
Started provisioning Amazon S3 repository "AWS Repository" target bucket prod-archives/vbr (session 459a5d6a-9146-4ee1-a0ef-02bfce8fbd3a).
Use veeamgo get session logs --id 459a5d6a-9146-4ee1-a0ef-02bfce8fbd3a to track progress.
```

**Validation error**
```
user@host % veeamgo add objectrepository amazon-s3 --name "Broken Repo"
Error: provide cloud credential ID using --credentials-id or include account.credentialsId in the spec
```
...
**Help**
```
user@host % veeamgo edit objectrepository --help
Edit an object storage repository

Usage:
  veeamgo edit objectrepository [flags]
...
Flags:
  ...
  -h, --help                 help for objectrepository
      ...
      --name string          Repository name to edit
      --new-name string      New repository name
      --set stringArray      Override value (repeatable, dot notation)
      --spec string          Path to JSON spec used as the base payload (optional)
      --type string          Expected repository type (optional)
      --wait                 Wait for the repository reconfiguration session to finish
      --yes                  Confirm without prompting (required to execute)
...
**Success**
```
user@host % veeamgo edit objectrepository --name "AWS Repository" --set bucket.folderName=infra --wait --yes
Amazon S3 Repository "AWS Repository" targeting bucket archive-bucket/infra update finished with result Success (session 9e75f3d1-3c3e-4f8d-83de-7fe8f00de8cf).
Repository ID: 1b6e9ba6-0739-4ae3-9c2a-1c5d57eafc4f
```

**Error**
```
user@host % veeamgo edit objectrepository --name "Default Backup Repository"
Error: repository "Default Backup Repository" is not an object storage repository; use veeamgo edit repository
```

### `veeamgo rescan objectrepository`

**Help**
```
user@host % veeamgo rescan objectrepository --help
Rescan object storage repositories

Usage:
  veeamgo rescan objectrepository [flags]

Flags:
      --all          Rescan every object storage repository
  -h, --help         help for objectrepository
      --id strings   Repository ID to rescan (can be repeated)
      --name strings Repository name to rescan (can be repeated)
      --wait         Wait for the rescan session to finish
...
**Success**
```
user@host % veeamgo rescan objectrepository --name "AWS Repository"
id:                459a5d6a-9146-4ee1-a0ef-02bfce8fbd3a
name:              Object repository rescan
state:             Running
result:
progressPercent:   0
creationTime:      2025-10-17 11:42:00
```

**Error**
```
user@host % veeamgo rescan objectrepository --name "Default Backup Repository"
Error: repository "Default Backup Repository" is not an object storage repository; use veeamgo rescan repository
```
...
**Help**
```
user@host % veeamgo delete objectrepository --help
Delete an object storage repository

Usage:
  veeamgo delete objectrepository [flags]
...
**Success**
```
user@host % veeamgo delete objectrepository --name "AWS Repository" --yes
Deleted Amazon S3 Repository "AWS Repository" (1b6e9ba6-0739-4ae3-9c2a-1c5d57eafc4f).
```

**Error**
```
user@host % veeamgo delete objectrepository --name "Default Backup Repository"
Error: repository "Default Backup Repository" is not an object storage repository; use veeamgo delete repository
```
...
Usage:
  veeamgo describe job --name <name> [flags]
...
**Success**
```
user@host % veeamgo describe job --name "Backup Job 1"
Name:             Backup Job 1
Type:             VSphereBackup
Workload:         vm
...
Object Name                Type       Platform        Created                       Job Name         Malware Status  Id
linbase02 AgentBackupJob   Increment  LinuxPhysical   2024-11-06 00:30:20 (+01:00)  AgentBackupJob   Clean           8b9f1c47-0d29-4e6e-ae1a-4f2131f948d1
```
...
Flags:
  ...
  -h, --help             help for proxy
      ...
      --type string      Filter by proxy platform (vmware, hyperv, general)
...
Flags:
  -h, --help          help for proxy
      ...
      --type string   Proxy platform to disambiguate when resolving the name (vmware, hyperv, general)
...
**Help**
```
user@host % veeamgo add proxy --help
Add a backup proxy

Usage:
  veeamgo add proxy [flags]

Flags:
      --auto-select-datastores         Automatically select accessible datastores (VMware proxies only)
      --description string             Proxy description
      --failover-to-network            Allow fallback to network mode if the primary transport fails (VMware proxies only)
  -h, --help                           help for proxy
      --host-to-proxy-encryption       Enable TLS encryption between host and proxy in network mode (VMware proxies only)
      --managed-server string          Managed server host name already added to inventory
      --max-tasks int                  Maximum concurrent tasks handled by the proxy (default: platform default)
      --name string                    Proxy name (defaults to managed server name if omitted)
      --transport-mode string          Transport mode for VMware proxies (auto, directAccess, virtualAppliance, network) (default "auto")
      --type string                    Proxy platform (vmware, hyperv, general) (default "vmware")
      --wait                           Wait for the provisioning session to complete
      --yes                            Confirm without prompting (required for non-interactive use)
...
**Success**
```
user@host % veeamgo add proxy --managed-server hyperv01.lab.local --type hyperv
Add Microsoft Hyper-V Proxy "hyperv01.lab.local" on host "hyperv01.lab.local" [y/N]: y
Started provisioning Microsoft Hyper-V Proxy "hyperv01.lab.local" on host "hyperv01.lab.local" (session 7c8a4c96-7f12-4094-a4bd-219ac7ebc1fe).
Use veeamgo get session logs --id 7c8a4c96-7f12-4094-a4bd-219ac7ebc1fe to track progress.
```

**Error**
```
user@host % veeamgo add proxy --type vmware
Error: flag --managed-server is required (provide the managed server host name)
Usage:
  veeamgo add proxy [flags]
```
...
**Help**
```
user@host % veeamgo edit proxy --help
Edit a backup proxy

Usage:
  veeamgo edit proxy [flags]
...
Flags:
  ...
  -h, --help                          help for proxy
      --host-to-proxy-encryption      Enable TLS encryption between host and proxy in network mode (VMware proxies only)
      --id string                     Proxy ID to edit (optional)
      --managed-server string         Managed server host name to associate with the proxy
      --max-tasks int                 Maximum concurrent tasks handled by the proxy
      --name string                   Proxy name to edit (optional)
      --new-name string               New proxy name
      --no-auto-select-datastores     Disable automatic datastore selection (VMware proxies only)
      --transport-mode string         Transport mode for VMware proxies (auto, directAccess, virtualAppliance, network)
      --type string                   Proxy platform to disambiguate by name (vmware, hyperv, general)
      --wait                          Wait for the proxy reconfiguration session to finish
      --yes                           Confirm without prompting (required to execute)
...
**Success**
```
user@host % veeamgo edit proxy --name "VMware vSphere Backup Proxy" --type vmware --description "Primary vCenter transport" --max-tasks 4 --wait --yes
VMware Proxy "VMware vSphere Backup Proxy" on host "vcenter01.lab.local" update finished with result Success (session 3ad8ec0b-8e45-4fe8-9f0d-3f6b6f0e1251).
Proxy ID: 18b661c1-d9dc-4233-90a0-7e7b10dc2d09
```

**Error**
```
user@host % veeamgo edit proxy --id 18b661c1-d9dc-4233-90a0-7e7b10dc2d09
No changes supplied; VMware Proxy "VMware vSphere Backup Proxy" left unchanged.
```
...
**Help**
```
user@host % veeamgo delete proxy --help
Delete a backup proxy

Usage:
  veeamgo delete proxy [flags]

Flags:
      --id string    Proxy ID to delete (optional)
  -h, --help         help for proxy
      --name string  Proxy name to delete (optional)
      --type string  Proxy platform to disambiguate by name (vmware, hyperv, general)
      --yes          Confirm without prompting (required to execute)
...
**Success**
```
user@host % veeamgo delete proxy --name "VMware vSphere Backup Proxy" --type vmware
Permanently delete VMware Proxy "VMware vSphere Backup Proxy" on host "vcenter01.lab.local" [y/N]: y
Deleted VMware Proxy "VMware vSphere Backup Proxy" (18b661c1-d9dc-4233-90a0-7e7b10dc2d09).
```

**Error**
```
user@host % veeamgo delete proxy
Error: provide --name or --id
Usage:
  veeamgo delete proxy [flags]
```
...
**Help**
```
user@host % veeamgo enable proxy --help
Enable a backup proxy

Usage:
  veeamgo enable proxy [flags]

Flags:
      --id string    Proxy ID to enable (optional)
  -h, --help         help for proxy
      --name string  Proxy name to enable (optional)
      --type string  Proxy platform to disambiguate by name (vmware, hyperv, general)
      --yes          Confirm without prompting (required to execute)
...
**Success**
```
user@host % veeamgo enable proxy --name "VMware vSphere Backup Proxy" --type vmware --yes
Enabled VMware Proxy "VMware vSphere Backup Proxy" (18b661c1-d9dc-4233-90a0-7e7b10dc2d09).
```

**Error**
```
user@host % veeamgo enable proxy
Error: provide --name or --id
Usage:
  veeamgo enable proxy [flags]
```
...
**Help**
```
user@host % veeamgo disable proxy --help
Disable a backup proxy

Usage:
  veeamgo disable proxy [flags]

Flags:
      --id string    Proxy ID to disable (optional)
  -h, --help         help for proxy
      --name string  Proxy name to disable (optional)
      --type string  Proxy platform to disambiguate by name (vmware, hyperv, general)
      --yes          Confirm without prompting (required to execute)
...
**Success**
```
user@host % veeamgo disable proxy --name "VMware vSphere Backup Proxy" --type vmware
Disable VMware Proxy "VMware vSphere Backup Proxy" on host "vcenter01.lab.local" [y/N]: y
Disabled VMware Proxy "VMware vSphere Backup Proxy" (18b661c1-d9dc-4233-90a0-7e7b10dc2d09).
```

**Error**
```
user@host % veeamgo disable proxy --name "VMware vSphere Backup Proxy" --type vmware --yes
VMware Proxy "VMware vSphere Backup Proxy" is already disabled.
```
