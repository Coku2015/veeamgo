...
- Commands follow the shape `veeamgo <verb> <resource>` with `get` preferred for read-only flows.
...
- Each resource exposes:
  ...
  - `describe` for detailed views (`veeamgo describe job --name <name>`).
  - Optionally scoped listing flags such as `--job <id>` or `--server <id>`.
- Early milestones must include:
  1. `login`, `logout`, `get session`, `session tokens`
  2. `veeamgo get job`, `veeamgo describe job --name <name>`
